...
class SecSignIDSoapApi
{
    ...
    /**
     * If the session was accepted and is not used any longer it can be released. 
     * Otherwise the session will be removed at server side automatically after some time.
     */
    function releaseAuthSession($authSessionId){
		...
		$soapData = $this->__encsoap($soapDataArray);
		$soapResponse = $this->secpki->__soapCall(
    						// "seccommerce.secappserver.pki.PKIUserSecAppServer.releaseAuthSession",
    						"releaseAuthSession", 
    						array(new SoapParam($soapData, "seccommerce.mobile.bo.ReqReleaseAuthSession")));
